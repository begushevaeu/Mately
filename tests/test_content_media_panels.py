...
@pytest.mark.asyncio
async def test_content_panel_edits_caption_for_photo_notifications() -> None:
    message = FakePhotoMessage()

    await edit_content_panel(message, "Оценка", reply_markup="keyboard")
...
@pytest.mark.asyncio
async def test_content_panel_from_state_edits_caption_for_remembered_photo_notifications() -> None:
    message = FakePhotoMessage()
    state = FakeState()
    bot = FakeBot()

    await remember_content_panel_in_state(state, message)
    await edit_content_panel_from_state(bot, state, "Выбери реакцию", reply_markup="keyboard")

    assert bot.edited_text is None
    assert bot.edited_caption == {
        "chat_id": 100,
        "message_id": 42,
        "caption": "Выбери реакцию",
        "reply_markup": "keyboard",
        "parse_mode": "HTML",
    }

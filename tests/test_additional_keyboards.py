...
def test_additional_menu_contains_statistics_settings_and_close() -> None:
    ...
    assert [button.text for button in buttons] == ["📊 Статистика", "📤 Экспорт", "⚙️ Настройки", "Закрыть"]
    assert buttons[0].callback_data == ADDITIONAL_STATISTICS_CALLBACK
    assert buttons[1].callback_data == ADDITIONAL_EXPORT_CALLBACK
    assert buttons[2].callback_data == ADDITIONAL_SETTINGS_CALLBACK
    assert buttons[3].callback_data == close_block_callback(ADDITIONAL_BLOCK_KEY)
